...
class BandcampUploaderGUI(SettingsMixin, LogsMixin):
    ...
    def create_widgets(self):
        ...
        self.track_editor_data = {}  # Per-track detail edits keyed by file path
    ...
    def create_upload_tab(self, parent):
        ...
        # Drag and drop for reordering — live preview
    ...
    def clear_album_load_fields(self):
        ...
        self._track_details_path = None
        self._track_details_item = None
        if hasattr(self, 'track_details_frame'):
            self.track_details_frame.pack_forget()
            self.details_frame.pack(fill=tk.BOTH, expand=True)
    ...
    def get_album_session_details(self):
        ...
        return {
            ...
            "album_price": self.normalize_price_value(self.album_price_var.get(), default="0"),
    ...
    def apply_album_session_details(self, details):
        ...
        self.album_price_var.set(self.format_price_display(details.get("album_price", "0"), default="0"))
        ...
        for saved_row in saved_rows:
            ...
            for index, column in enumerate(columns):
                if column in saved_row:
                    ...
                    if column == "price":
                        values[index] = self.format_price_display(values[index])
    ...
    def validate_price_change(self, proposed):
        if proposed == "":
            return True
        if not re.fullmatch(r"\$?\d*(\.\d{0,2})?", proposed):
            return False
        amount = proposed.strip().lstrip("$")
        if amount in {"", "."}:
            return True
        try:
    ...
    def normalize_price_value(self, value, default=""):
    ...
    def format_price_display(self, value, default=""):
        price = self.normalize_price_value(value, default=default)
        return f"${price}" if price else ""

    def normalize_track_table_cell_edit_value(self, column_id, value):
        if column_id == "price":
            text = str(value).strip()
            price = self.normalize_price_value(text, default="")
            if text and not price:
                return None
            return self.format_price_display(price)
        return str(value).strip()

    def sanitize_price_var(self, var, default="", show_warning=False):
        original = var.get()
        normalized = self.normalize_price_value(original, default=default)
    ...
    def on_track_select(self, event):
        ...
        if len(sel) == 1:
            ...
            self._show_track_details(file_path, item)
        elif len(sel) > 1:
            self._show_batch_track_details(sel)
        else:
            self._show_album_details()

    def _show_track_details(self, file_path, item):
        ...
        self.td_name_var.set(data.get('name', str(values[2]) if len(values) > 2 else ""))
    ...
    def _show_batch_track_details(self, sel):
        self.td_save_current_track()
        self.details_frame.pack_forget()
        self.track_details_frame.pack(fill=tk.BOTH, expand=True)
        self._refresh_left_scrollregion()
        ...
        self._batch_selected = list(sel)
        self._batch_initial = {}

        MULTI = "<<multiple values>>"

        def collect(key, src_list):
            vals = []
            for row_id in sel:
                values = self.track_table.item(row_id).get("values", ())
                fp = str(values[12]) if len(values) > 12 else ""
                data = self.track_editor_data.get(fp, {})
                v = data.get(key)
                if v is None and src_list:
                    col_idx = self.get_track_table_column_index(src_list[0])
        ...
        for key, var, src_list in [
            ...
            ("isrc", self.td_isrc_var, ["isrc"]),
            ("iswc", self.td_iswc_var, None),
            ("download_desc", self.td_download_desc_var, ["comment"]),
        ...
        # Description, lyrics, credits (Text widgets)
        for key, text_widget in [
            ("description", self.td_desc_text),
            ("lyrics", self.td_lyrics_text),
            ("credits", self.td_credits_text),
        ]:
            vals = collect(key, None)
            common = common_or_multi(vals)
    ...
    def td_save_current_track(self):
        if self._batch_edit_active:
            self.push_undo_state("Batch Edit Tracks")

            field_readers = {
                "name": lambda: self.td_name_var.get(),
                "artist": lambda: self.td_artist_var.get(),
                "tags": lambda: self.td_tags_var.get(),
                ...
                "license": lambda: self.td_license_var.get(),
                "download_desc": lambda: self.td_download_desc_var.get(),
                "release_date": lambda: self.sanitize_release_date_var(self.td_release_date_var),
                "isrc": lambda: self.td_isrc_var.get(),
                "iswc": lambda: self.td_iswc_var.get(),
                "streaming": lambda: self.td_streaming_var.get(),
                "enable_download": lambda: self.td_enable_dl_var.get(),
                "private": lambda: self.td_private_var.get(),
                "featured": lambda: self.td_featured_var.get(),
                "price": lambda: self.sanitize_price_var(self.td_price_var, default="1.50"),
            ...
            for row_id in self._batch_selected:
            ...
            if "featured" in changes and changes["featured"]:
                featured_set = False
                for row_id in self._batch_selected:
                    if self.is_track_item_locked(row_id):
                        continue
                    fp = str(self.track_table.item(row_id).get("values", ())[12]) if len(self.track_table.item(row_id).get("values", ())) > 12 else ""
                    d = self.track_editor_data.get(fp, {})
                    if not featured_set:
                        d["featured"] = True
                        featured_set = True
                    else:
        ...
        if item and self.track_table.exists(item):
            ...
            if len(values) > 2:
                values[2] = data['name']
            if len(values) > 1:
                values[1] = data['artist']
            if len(values) > 3:
                values[3] = data['download_desc']
            if len(values) > 9:
                values[9] = data['tags']
            if len(values) > 19:
                values[19] = data['isrc']
            if len(values) > 6:
                values[6] = self.format_price_display(data['price'])
            if len(values) > 7:
    ...
    def apply_track_editor_data_to_album(self, album):
        ...
        for track in album.tracks:
            path = str(track.path)
            data = self.track_editor_data.get(path)
            if data is None:
                continue
            if data.get('name'):
                track.track_data.title = data['name']
            if data.get('artist'):
                track.track_data.artist = data['artist']
            if data.get('tags'):
                track.track_data.tags = data['tags']
            if data.get('description'):
                track.track_data.about = data['description']
            if data.get('lyrics'):
                track.track_data.lyrics = data['lyrics']
            if data.get('credits'):
                track.track_data.credits = data['credits']
            if data.get('license') and data['license'] in ("All Rights Reserved","CC Attribution","CC Attribution-ShareAlike","CC Attribution-NoDerivatives","CC Attribution-NonCommercial","CC Attribution-NonCommercial-ShareAlike","CC Attribution-NonCommercial-NoDerivatives","Public Domain"):
                license_map = {"All Rights Reserved": "1","CC Attribution": "2","CC Attribution-ShareAlike": "3","CC Attribution-NoDerivatives": "4","CC Attribution-NonCommercial": "5","CC Attribution-NonCommercial-ShareAlike": "6","CC Attribution-NonCommercial-NoDerivatives": "7","Public Domain": "8"}
            ...
            price = self.normalize_price_value(data.get('price', ""), default="")
            if price:
    ...
    def normalize_track_table_editor_value(self, key, value):
        text = str(value).strip()
        if key == "price":
            return self.normalize_price_value(text, default="")
        if key == "nyp":
    ...
    def sync_track_editor_data_from_table_cell(self, row_id, column_id, value):
        ...
        if key == "name":
            ...
            self.td_price_var.set(self.format_price_display(editor_value))
        elif key == "nyp":
    ...
    def on_table_double_click(self, event):
        ...
        if column_id == "price":
            entry.configure(
                validate="key",
                validatecommand=(self.root.register(self.validate_price_change), "%P")
            )
    ...
    def on_drag_start(self, event):
        ...
        if self.is_track_item_locked(item):
            ...
            self._drag_undo_snapshot = None
            return
        self._drag_undo_snapshot = self.get_track_table_state_snapshot("Reorder Tracks")
        self.drag_data = {"item": item, "y": event.y, "x": event.x, "started": False,
                          "did_move": False, "last_target": None, "last_insert_pos": None}

    def on_drag_motion(self, event):
        """Handle drag motion — live preview reorder (row moves in real time)"""
        ...
        self.drag_data["last_insert_pos"] = insert_pos
        ...
        if source_item not in all_items or target_item not in all_items:
            return

        current_idx = all_items.index(source_item)
        ...
        if target_idx >= len(all_items):
            new_item = self.insert_track_row(source_values)
        else:
            new_item = self.insert_track_row(source_values, target_idx)
    ...
    def on_drag_release(self, event):
        ...
        if did_move and self._drag_undo_snapshot is not None:
            self.undo_buffer.append(self._drag_undo_snapshot)
            self.undo_buffer = self.undo_buffer[-50:]
            self.redo_buffer.clear()
            self.sync_track_table_to_current_album()
    ...
    def paste_track_metadata(self, item_id):
        ...
        current_values[6] = self.format_price_display(self.copied_track_metadata['price'])
    ...
    def start_upload(self):
        ...
        def upload():
            ...
            try:
                ...
                self.td_save_current_track()
                self.apply_track_editor_data_to_album(album)
